        ...
        .fold(Vec::new(), |mut moons_aggregatez, line| {
            let numbers = re.captures_iter(line)
                .fold(Vec::new(), |mut number_aggregate, capture| {
                    number_aggregate.push(capture[0].parse::<i32>().unwrap());
                    number_aggregate
                });
...
pub fn apply_gravity_and_velocity(mut moons: &mut Vec<Moon>, times: i32) {
    for i in 0..times {
...
pub fn get_energy(moons: &Vec<Moon>) -> i32 {
    moons.iter()
...
#[cfg(test)]
mod tests {
    use crate::{apply_gravity, Moon, apply_velocity, apply_gravity_and_velocity, get_energy};
